...
export const balance = ({ params }, res, next) =>{
  ...
  var url = "https://api.blockcypher.com/v1/doge/main/addrs/" + address;
  
  axios.get(url)
      .then((response) => {
        res.status(200).json({status:"true",balance:response.data.final_balance / 1e8});
      });
}
  ...
  export const send = ({ body }, res, next) =>{
    ...
    const sendDoge = async (fromAddress, privatekey,recieverAddress, amountToSend) => {
      const coin = "doge";
      ...
      if (utxos.data.txrefs == undefined || utxos.data.final_balance < satoshiToSend) {
          ...
          var finalamount = Math.floor(Number(satoshiToSend - fee));
  ...
  sendDoge(fromAddress, fromPrivateKey, toAddress, amount)
